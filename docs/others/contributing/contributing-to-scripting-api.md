...
This documentation is intended at contributors who want to participate in the development of ArqueumSpace itself.
...
The philosophy behind ArqueumSpace is to build a platform that is as open as possible. Part of this strategy is to
offer map developers the ability to turn a ArqueumSpaces map into something unexpected, using the API. For instance,
you could use it to develop games (we have seen a PacMan and a mine-sweeper on ArqueumSpace!)

We started working on the ArqueumSpace scripting API with this in mind, but at some point, maybe you will find that
...
Extending the scripting API means modifying the core of ArqueumSpace. You can of course run these
...
The iframe allows ArqueumSpace to isolate the script in a sandbox. Because the iframe is sandbox (or on a different
domain than the ArqueumSpace server), scripts cannot directly manipulate the DOM of ArqueumSpace. They also cannot
directly access Phaser objects (Phaser is the game engine used in ArqueumSpace). This is by-design. Since anyone
can contribute a map, we cannot allow anyone to run any code in the scope of the ArqueumSpace server (that would be
a huge XSS security flaw).

Instead, the only way the script can interact with ArqueumSpace is by sending messages using the
...
When this script is called, the scripting API is dispatching a JSON message to ArqueumSpace.
...
In ArqueumSpace, the message is received in the [`IframeListener` listener class](http://github.com/thecodingmachine/workadventure/blob/1e6ce4dec8697340e2c91798864b94da9528b482/front/src/Api/IframeListener.ts#L200-L203).
This class is in charge of analyzing the JSON messages received and dispatching them to the right place in the ArqueumSpace application.
...
In the example above, we already saw you can easily send a message from the iframe to ArqueumSpace using the
...
Of course, messaging can go the other way around and ArqueumSpace can also send messages to the iframes.
...
from the iframe in ArqueumSpace, and to expect a response. For those use cases, the iframe API comes with a
...
is an entry point to ArqueumSpace, and as with any entry point, data must be checked (otherwise, a hacker could
send specially crafted JSON packages to try to hack WA).

In ArqueumSpace, we use the [zod](https://github.com/colinhacks/zod) package. This package
...
The advantage of this technique is that, **at runtime**, ArqueumSpace can verify that the JSON message received
...
For "one-way" messages (from the iframe to ArqueumSpace), the `sendToWorkadventure` method expects the passed
...
Similarly, if you want to type messages from ArqueumSpace to the iframe, there is a very similar `IframeResponseEvent`.
...
### Responding to a query on the ArqueumSpace side

In the ArqueumSpace code, each possible query should be handled by what we call an "answerer".
...
- `sendToWorkadventure` is used to send messages one way from the iframe to ArqueumSpace. No response is expected. In particular
  if an error happens in ArqueumSpace, the iframe will not be notified.
- `queryWorkadventure` is used to send queries that expect an answer. If an error happens in ArqueumSpace (i.e. if an
