...
1. **Teste de contrato** por endpoint: a resposta valida contra o mesmo schema `zod` que o pusher usa (`isCapabilities`, `isMapDetailsData`, `isRoomRedirect`, `isFetchMemberDataByUuidResponse`, `wokaList`). *Reusar os schemas de `@workadventure/messages` — não redigitar.*
   ...
   >
   > O P3 precisou da mesma mudança mais duas vezes (2026-08-04, [ADR-0005](0005-moderation.pt-BR.md)): o `AdminBannedData` — que estava definido **duas vezes**, no `AdminApi.ts` e no `SharedAdminApi.ts` — e o `ShortMapDescription`. O segundo faz merge do `WAMMetadata`, do `@workadventure/map-editor`, e esse pacote já depende do `messages`, então o `WAMMetadata` também foi movido e é re-exportado do `types.ts`. De novo, nenhum import mudou em lugar nenhum.
