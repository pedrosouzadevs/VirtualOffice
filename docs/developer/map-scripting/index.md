...
Please note that scripting in ArqueumSpace is evolving with each release.
...
evolve in the future versions of ArqueumSpace. Use those with caution. You can actually voice your opinion about useful features by adding [an issue on GitHub](https://github.com/thecodingmachine/workadventure/issues).
...
*   **In an iFrame**: By placing your Javascript script into an iFrame, your script can communicate with the ArqueumSpace game
...
:::info Script types
ArqueumSpace supports two types of scripts:

1. **JavaScript files (`.js` or `.ts`)**: The script will be automatically wrapped in an HTML page with the ArqueumSpace API (`iframe_api.js`) included. You should use this most of the time.
2. **HTML files (`.html`)**: The HTML file will be loaded directly as-is. You need to manually include the ArqueumSpace API if you want to use it:
   ```html
   <script src="https://play.workadventu.re/iframe_api.js"></script>
   ```
   
The HTML page is hidden, so don't expect to see it somewhere. If you want to display custom HTML elements within ArqueumSpace, take a look at the dedicated functions
...
The `WA` objects contains a number of useful methods enabling you to interact with the ArqueumSpace game. For instance, `WA.chat.sendChatMessage` opens the chat and adds a message in it.

The message should be displayed in the chat history as soon as you enter the room.

To learn more about the way scripts are loaded by ArqueumSpace (and in particular if you are facing) CORS issues,
please read the [scripting internals](./scripting-internals) guide that dives into the specifics of how ArqueumSpace
loads your scripts in a secure way.

## Adding a script in an iFrame

In ArqueumSpace, you can easily [open an iFrame using the `openWebsite` property on a layer](../../map-building/tiled-editor/special-zones). However, by default, the iFrame is not allowed to communicate with ArqueumSpace.

This is done to improve security. In order to be able to execute a script that communicates with ArqueumSpace inside an iFrame, you have to **explicitly allow the iFrame to use the "iFrame API"**.

In order to allow communication with ArqueumSpace, you need to add an additional property: `openWebsiteAllowApi`. This property must be _boolean_ and you must set it to "true".

![The openWebsiteAllowApi property](images/open_website_allow_api.png)

In your iFrame HTML page, you now need to import the _ArqueumSpace client API Javascript library_. This library contains the `WA` object that you can use to communicate with ArqueumSpace.

The library is available at `https://play.workadventu.re/iframe_api.js`.

_Note:_ if you are using a self-hosted version of ArqueumSpace, use `https://[front_domain]/iframe_api.js`
