...
:::caution
ArqueumSpace has an aggressive optimization mechanism where your client will only receive information from players
...
ArqueumSpace only tracks players close to you. So `onPlayerEnters` will only be triggered when a remote player
...
In this case, instead of subscribing to each and every player, ArqueumSpace offers
a shortcut method:
