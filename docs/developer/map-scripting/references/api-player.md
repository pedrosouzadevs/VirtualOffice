...
of ArqueumSpace, you can define tags related to the user in the [administration panel](/admin/members).
:::
...
The token is generated by the administration panel linked to ArqueumSpace. The token is a string and is depending on your implementation of the administration panel.
...
The token can be used as a way to authenticate a ArqueumSpace player and ensure he is indeed in the map and authorized to upload a logo.
:::
...
- Transient variables disappear as soon as the connection to the room is lost. So if you
  ...
  if you simply close ArqueumSpace and come back later, the transient player variable value
...
player variables. So far, ArqueumSpace SAAS version has no maximum TTL set. However, please note you should
...
If you are using the SAAS version (online version) of ArqueumSpace, you can create your worlds from the admin dashboard
and put your rooms in those worlds.

If you are using the self-hosted version of ArqueumSpace (with no custom admin API configured), there is only one world, 
...
the ArqueumSpace server. If the player is not logged, the player variables are stored in local storage (so in the
player's browser).
:::
...
You can be connected several times with the same user to ArqueumSpace (and ArqueumSpace
will not complain about it, this is by design).
Open another tab, connect again to ArqueumSpace and you will be connected to ArqueumSpace
twice with the same user. We will call those users connected several times to ArqueumSpace
**brothers**.
...
voice chat in ArqueumSpace. The sound can be generated on a server and streamed to the players in the bubble.
