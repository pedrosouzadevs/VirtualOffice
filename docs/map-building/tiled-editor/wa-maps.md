...
# About ArqueumSpace maps

A ArqueumSpace map is a map in "JSON" format generated by [Tiled](https://www.mapeditor.org/).

## Tiles

A map is made of "tiles" (we can also call them "sprites"). In ArqueumSpace, the tiles are small images of 32x32 pixels.
...
## ArqueumSpace Map Rules

In order to design a map that will be readable by ArqueumSpace, you will have to respect some constraints.

In particular, you will need to:

*   set a start position for the players
*   configure the "floor layer" (so that ArqueumSpace can correctly display characters above the floor, but under the ceiling)
...
*   ArqueumSpace doesn't support object layers and will ignore them
