...
[Upload your map to the ArqueumSpace server](./wa-hosted).
...
<div class="text--center margin-top--lg">
    <a href="./wa-hosted" class="button button--primary">Upload your map to the ArqueumSpace server &gt;</a>
</div>

## Legacy options

In the past, ArqueumSpace could not host maps directly on its server. Therefore, we had to rely on third-party websites for map hosting.
These options are still supported, but they are not recommended for new projects. They are more complex to set up and maintain, and they do not offer any significant advantage over hosting on the ArqueumSpace server.
...
flowchart TB
  subgraph Self-hosted
    ...
    WA3[ArqueumSpace] -. load map .-> WS
  end
  subgraph GitHub pages
    direction BT
    Y2[Your computer] -- push --> GH[GitHub]
    GH[GitHub] -- GitHub actions --> GHP[GitHub Pages]
    WA2[ArqueumSpace] -. load map .-> GHP
  end
  subgraph ArqueumSpace
    direction BT
    Y1[Your computer] -- upload map --> WA1[ArqueumSpace]
  end
