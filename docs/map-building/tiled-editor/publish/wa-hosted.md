---
sidebar_position: 10
title: Upload your Map to ArqueumSpace
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Upload your Map to ArqueumSpace
...
In the following sections, we will explain how to upload your maps to the ArqueumSpace 'map storage' server from [the starter-kit web app](#using-the-starter-kit-web-app), from [the command line](#using-the-command-line) or [from a CI/CD pipeline](#using-github-and-a-cicd-pipeline).
...
Where you get this information depends on the kind of ArqueumSpace version you are using (SaaS version or self-hosted).

Where you get this depends on the kind of ArqueumSpace server you are using.
If you are using the SaaS version of ArqueumSpace, then you will find the information on the Admin panel.
If you are self-hosting ArqueumSpace, then you will have to configure the map storage yourself.
...
1. **The URL of your map storage:**
   ...
   There are 3 links, be careful to take the `Map-storage API endpoint`, it is the url for uploading files to the map storage service of ArqueumSpace.
...
The system administrator that installed the ArqueumSpace server should have provided you with an API key.
...
ArqueumSpace server. This is the simplest way to upload your maps.
...
A click on the upload button will trigger the upload of your map to the ArqueumSpace server. Please be patient as before the
...
When you upload your map, you can specify a "directory" where your map files will be stored in the ArqueumSpace server.

:::info
When you upload your files, all the maps in your "starter-kit" project will be uploaded to the ArqueumSpace server at once.
...
When you upload files to the ArqueumSpace server, all the files previously uploaded in the same directory will be removed
...
:::info
You can have several projects (started from several starter kits), stored in different directories, uploaded to the same ArqueumSpace world.
...
uploaded maps (self-hosted version). You can click on any of these maps to access them in ArqueumSpace.
...
From here, you will see your map listed in the "world" section. Just click the map you want to access, and it will open in ArqueumSpace.
...
you uploaded, ArqueumSpace will create a `.wam` file that contains the map data and any modifications made to the map
using the ArqueumSpace Map Editor.
...
2. Then, a ZIP file of the dist directory is created and sent to the ArqueumSpace "map-storage" server. This server is in charge of hosting the map files. When it receives the ZIP file, it unzips it and stores the files in the directory you configured as 'Upload directory'. For each `.tmj` file the server finds, it will check if there exists a matching `.wam` file. If not, it will create one. WAM files are used to store any part of the map edited by the Map editor of ArqueumSpace (like the list of objects or areas, the microphone settings, etc.)

:::danger Backup your original files!
If you're uploading on ArqueumSpace server, it only stores the "build" you send to it. It does not store the original files you used to create the map. If you want to update your map, you need to update the original files on your computer and run the `npm run upload` command again. So do not think you can get back the original files from the ArqueumSpace server. It is your responsibility to store the original map files in a safe place in case you want to modify those (like GitHub).
:::

:::info Wrapping things up
In this chapter, we saw how to upload your map from your computer to the ArqueumSpace server using a command line.
...
- you can use a **CI/CD pipeline** (like "GitHub Actions") to automatically upload your map to the ArqueumSpace server when you push changes to the repository.

The map starter kit comes with a GitHub Actions workflow that is designed to upload your map to the ArqueumSpace server when you push your changes to the `master` branch of your repository. It also means you can now have a private repository instead of a public one, because we won't use GitHub Pages anymore (GitHub free hosting).
...
Remember the map(s) uploaded to the ArqueumSpace server is the content of the `dist` directory. If things are working locally, but act strangely on the server (sound files not playing, HTTP 404 errors, etc...), you should check the content of the `dist` directory.
...
When you push changes to your repository, the GitHub Actions workflow is triggered. It will build your map and upload it to the ArqueumSpace server.
...
If you are facing any troubles, do not hesitate to seek help in [our Discord server](https://discord.gg/G6Xh9ZM9aR) or open an issue in the [GitHub ArqueumSpace project](https://github.com/workadventure/workadventure/issues).
