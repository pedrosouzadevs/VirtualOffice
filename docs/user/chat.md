...
2 types of chat rooms are available in ArqueumSpace:
...
Matrix chat rooms take their name from the Matrix protocol, which is used to provide chat features in ArqueumSpace.
...
Matrix account can join a public room. Even if they are not logged in ArqueumSpace.
:::
...
It means nobody except the participants of the room can read the messages. Not even the ArqueumSpace server administrators.
...
want to invite users with a Matrix ID outside ArqueumSpace to join).
...
:::info
If you are using the SAAS version of ArqueumSpace, there is another way to create a Matrix room that [automatically
...
The folder notion in ArqueumSpace is usually called a "space" in other Matrix client.

### Invitations

In order to open a chat with another user in your ArqueumSpace world, you need to invite them to a chat room.
...
## Meeting invitations
ArqueumSpace allows users to invite other users to join their meeting directly from the chat interface.
...
End-to-end encryption brings the highest level of security, ensuring that even ArqueumSpace administrators cannot read 
your messages. However, this means users become responsible for keeping their encryption key safe.

The first time you use encryption, ArqueumSpace generates an encryption key that can be saved using a recovery passphrase.
Be sure to store this key securely or remember the recovery passphrase.

Since the encryption key is stored in your browser and never reaches ArqueumSpace servers, if you access ArqueumSpace 
from another browser, you'll need to provide this encryption key again to read your message history.

To keep ArqueumSpace simple and avoid burdening first-time users, encryption is not enabled by default when you create 
a room or chat with a user. Enabling encryption is as easy as checking the "Active end-to-end encryption" box when creating
a room.

Please note that if you receive a message while you're away, ArqueumSpace will send a **notification email**. In encrypted 
rooms, ArqueumSpace cannot include a conversation snippet in the email because we can't decrypt the content. 
