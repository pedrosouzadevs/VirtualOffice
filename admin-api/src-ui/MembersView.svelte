<script lang="ts">
    ...
    interface Props {
        /** The acting administrator, so a row can tell when it is about the person looking at it. */
        me: Member | null;
        /** Kept in the shell so the header can show a name that a rename here has just changed. */
        onMeChanged: (member: Member) => void;
    }

    const { me, onMeChanged }: Props = $props();
